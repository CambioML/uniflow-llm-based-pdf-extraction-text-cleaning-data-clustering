...
from uniflow.flow.transform.transform_huggingface_flow import HuggingFaceModelFlow  # Update the import path as necessary
from uniflow.node import Node
from uniflow.op.prompt import PromptTemplate
from uniflow.op.model.lm.model import LmModel

class TestHuggingFaceModelFlow(unittest.TestCase):
    @patch('uniflow.flow.transform.transform_huggingface_flow.ModelOp')  # Update the import path as necessary
    @patch('uniflow.flow.transform.transform_huggingface_flow.LmModel')  # Update the import path as necessary
    def setUp(self, mock_lm_model, mock_model_op):
        ...
        self.huggingface_flow = HuggingFaceModelFlow(self.prompt_template, self.model_config)
    ...
    def test_init(self):
        ...
        self.mock_model_op.assert_called_once_with(
            ...
            model=self.mock_lm_model.return_value
        )
